...
import {
  Waypoints, Construction, Flame, ChevronRight, MapPin, AudioLines,
  Play, Check, Loader, Flag, ThumbsUp, MessageCircle,
  BadgeCheck, Send, Share2, Menu,
} from "lucide-react";

/**
 * /issues/[id] — Public issue detail page with its own minimal nav.
 * (Separate from the authenticated citizen route /citizen/issues/[id])
 */
export default function IssueDetailPage() {
  return (
    <div className="bg-[#F8FAF8] text-zinc-950 min-h-screen">
      {/* ── Minimal public header ────────────────────────────── */}
      <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-sm border-b border-zinc-200 shadow-sm">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-8 flex justify-between items-center h-[64px] sm:h-[72px]">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2 shrink-0">
            ...
            <div className="leading-none flex-col hidden sm:flex">
              <span className="font-bold text-zinc-950 text-sm leading-5 tracking-tight">Prathmikta Setu</span>
              <span className="text-[#71717b] text-[10px]">Public Issues</span>
            </div>
          </Link>

          {/* Breadcrumb — hidden on mobile */}
          <div className="hidden md:flex items-center gap-1.5 text-sm text-[#71717b] min-w-0 flex-1 mx-6">
            <Link href="/issues" className="hover:text-zinc-950 transition-colors shrink-0">Issues</Link>
            <ChevronRight className="size-4 shrink-0" />
            <Link href="#" className="hover:text-zinc-950 transition-colors shrink-0">Roads &amp; Infrastructure</Link>
            <ChevronRight className="size-4 shrink-0" />
            <span className="font-medium text-zinc-950 truncate">Severe Pothole &amp; Waterlogging on MG Road</span>
          </div>

          <div className="flex items-center gap-2">
            <button className="rounded-lg border border-zinc-200 px-3 py-1.5 text-sm flex items-center gap-2 hover:bg-zinc-50 transition-colors">
      ...
      <main className="max-w-[1200px] mx-auto px-4 sm:px-8 py-6">
        {/* Mobile breadcrumb */}
        <div className="flex items-center gap-1.5 text-xs text-[#71717b] mb-4 md:hidden flex-wrap">
          <Link href="/issues" className="hover:text-zinc-950">Issues</Link>
          <ChevronRight className="size-3" />
          <span className="text-zinc-950 font-medium truncate">Severe Pothole &amp; Waterlogging</span>
        </div>

        {/* Responsive: stacked on mobile, 12-col on xl */}
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
          ...
          <div className="xl:col-span-8 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-5 sm:p-6 flex flex-col gap-5">
              <div className="flex justify-between items-start gap-3 flex-wrap">
                <div className="flex items-start gap-3 min-w-0">
                  <div className="size-11 sm:size-12 shrink-0 rounded-xl bg-[#ff6900]/10 flex justify-center items-center">
                    <Construction className="size-5 sm:size-6 text-[#ff6900]" />
                  </div>
                  <div className="flex flex-col gap-2 min-w-0">
                    ...
                    <h1 className="font-bold text-lg sm:text-2xl tracking-tight leading-snug">
                      Severe Pothole &amp; Waterlogging on MG Road
                    </h1>
                  </div>
                </div>
                <span className="shrink-0 font-semibold rounded-full bg-amber-100 text-amber-700 text-xs px-3 py-1 flex items-center gap-1.5">
              ...
              <p className="text-[#71717b] text-sm leading-relaxed">
                ...
                During monsoon rainfall the entire junction becomes severely waterlogged, submerging the potholes and creating a
                serious hazard for two-wheelers and pedestrians. Multiple minor accidents have been reported over the past three weeks.
              </p>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-3">
                  {["Road damage", "Waterlogging", "More photos"].map((label, i) => (
                    <div
                      key={label}
                      className={`relative rounded-xl border border-zinc-200 overflow-hidden ${i === 0 ? "col-span-2" : ""} aspect-video bg-zinc-100 flex items-center justify-center`}
                    >
                ...
                <div className="rounded-xl border border-zinc-200 flex p-4 flex-col gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-end flex-1 gap-0.5 h-8 overflow-hidden">
                      {[3,5,7,4,6,3,5,7,4,6,3,5,4,6,3,5].map((h, i) => (
            ...
            <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-5 sm:p-6 flex flex-col gap-4">
              ...
              {[
                {
                  initials: "AK", name: "Anil Kumar", role: "Resident", time: "2 days ago",
                  comment: "This junction is genuinely dangerous after rain. Please prioritise the drainage as well, not just the surface patch.",
                  likes: 24,
                  official: { name: "PWD Roads Division", time: "1 day ago", reply: "Noted. Drainage survey added to work order. Resurfacing crew scheduled this week." },
                },
                {
                  initials: "PM", name: "Priya Menon", role: "Shopkeeper", time: "3 days ago",
                  comment: "Business has dropped because customers avoid this stretch. Glad to see it being addressed at last.",
                  likes: 18,
                },
              ].map((comment) => (
            ...
            <div className="xl:sticky xl:top-24 flex flex-col gap-4">
              ...
              <div className="bg-[#ff6900]/5 rounded-2xl border border-[#ff6900]/20 p-5">
                ...
                <p className="text-[#71717b] text-xs leading-relaxed">
                  ...
                  Road resurfacing + drainage repair recommended for lasting resolution.
                </p>
              </div>

              <Link
                href="/register"
                className="flex items-center justify-center gap-2 bg-[#ff6900] hover:bg-orange-600 text-orange-50 font-semibold text-sm px-5 py-3 rounded-2xl transition-colors"
              >
                Sign up to track &amp; upvote →
              </Link>
            </div>
          </div>
        </div>
      ...
      {/* Footer */}
      <footer className="border-t border-zinc-200 mt-8 bg-white">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-8 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2">
