  ...
  return (
    <CitizenLayout>
      <div className="max-w-3xl mx-auto flex flex-col gap-6">
        ...
        {/* Avatar card */}
        <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-6 flex items-center gap-5">
          ...
          <div>
            <h2 className="font-bold text-xl text-zinc-950">{fullName}</h2>
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            {STATS.map((s) => (
              <div
                key={s.label}
                className="rounded-xl border border-zinc-200 p-4 flex flex-col items-center gap-1 text-center"
              >
                <div className={`size-10 rounded-full ${s.bg} flex items-center justify-center`}>
                  <span className={`font-bold text-lg ${s.color}`}>{s.value}</span>
                </div>
                <span className="text-xs text-[#71717b] font-medium mt-1">{s.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-6 flex flex-col gap-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-2">
              <label className="font-medium text-sm text-zinc-950 flex items-center gap-1.5">
                <MapPin size={13} className="text-[#71717b]" />
                Ward / Location
              </label>
              <input
                value={ward}
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-6">
          ...
          <div className="flex flex-col gap-3">
            {RECENT_ACTIVITY.map((item) => (
              ...
              >
                <div
                  className={`size-9 rounded-full ${item.bg} flex items-center justify-center shrink-0`}
                >
                  {item.icon}
                </div>
                ...
                <div className="flex flex-col items-end gap-1 shrink-0">
                  <span className="text-[11px] text-[#71717b] flex items-center gap-1">
