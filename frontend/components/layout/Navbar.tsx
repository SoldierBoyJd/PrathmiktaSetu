...
export default function Navbar({ title, subtitle, alertCount }: NavbarProps) {
  ...
  const badgeCount =
    alertCount ?? MOCK_NOTIFICATIONS.filter((n) => n.unread).length;

  // Profile link depends on role
  const profileHref =
    user?.role === "citizen" ? "/citizen/profile" : "/mp/profile";
  ...
  return (
    <header className="fixed top-0 left-0 lg:left-[260px] right-0 h-[72px] bg-white/95 backdrop-blur-sm border-b border-zinc-200 z-20 flex items-center px-4 lg:px-8 justify-between gap-4">
      {/* Left — spacer for mobile hamburger + title/search */}
      <div className="flex items-center gap-3 min-w-0 flex-1">
        <div className="lg:hidden w-8 shrink-0" />

        {title ? (
          <div className="flex flex-col min-w-0">
            <h1 className="font-bold text-zinc-950 text-base sm:text-lg leading-7 truncate">
              {title}
            </h1>
            {subtitle && (
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <div className="relative" ref={notifRef}>
          <button
            ...
            className="relative size-10 rounded-lg border border-zinc-200 flex items-center justify-center hover:bg-zinc-50 transition-colors"
          >
            <Bell size={16} className="text-[#71717b]" />
            {badgeCount > 0 && (
              <span className="absolute top-1.5 right-1.5 size-[17px] rounded-full bg-[#ff6900] flex items-center justify-center">
                ...
                {MOCK_NOTIFICATIONS.map((n) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-zinc-950 leading-relaxed">
                        {n.text}
                      </p>
                      <p className="text-[11px] text-[#71717b] mt-0.5">
                        {n.time}
                      </p>
                    </div>
                  </div>
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            <div className="size-9 rounded-full bg-[#ff6900] flex items-center justify-center shrink-0">
              <span className="text-orange-50 font-semibold text-xs">
                {initials}
              </span>
            </div>
            <div className="leading-tight hidden sm:flex flex-col text-left">
              <span className="font-medium text-sm leading-5 text-zinc-950 max-w-[110px] truncate">
                {displayName}
              </span>
            ...
            <ChevronDown
              size={14}
              className={cn(
                "text-[#71717b] transition-transform",
          ...
          {dropdownOpen && (
            <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-2xl border border-zinc-200 shadow-lg overflow-hidden z-50">
              ...
              <div className="px-4 py-3 border-b border-zinc-100">
                <p className="font-semibold text-sm text-zinc-950 truncate">
                  {displayName}
                </p>
                <p className="text-[11px] text-[#71717b] truncate">
